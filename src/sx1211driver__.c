...
#define RF_SPI_ReedWrite SPI_B0_ReedWrite

volatile uint8_t PreMode = RF_STANDBY; //Previous chip operating mode
...
static void WriteRegister(uint8_t address, uint8_t value)
{
    ...
    RF_SPI_ReedWrite(address);
    RF_SPI_ReedWrite(value);
...
static uint8_t ReadRegister(uint8_t address)
{
    ...
    rcv_byte = RF_SPI_ReedWrite(address);
    rcv_byte = RF_SPI_ReedWrite(0);
    deselect_config_line();
...
{    
    ...
    select_data_line();
	rcv_byte = RF_SPI_ReedWrite(0x55);
    deselect_data_line();
...
void InitRFChip(RadioEvents_t *events)
{
    ...
    USCIB0_ToSpiMode(MASTER, 8);  //инициализация SPI B0
...
__interrupt void PortIRQ_Interrupt(void){
	...
	if( (IRQ_IE & IRQ_1_PIN) && (PreMode == RF_RECEIVER) ){
		BIT_CLEAR(IRQ_IFG, IRQ_1_PIN);
		ReadDataPacket(NULL);
	}
